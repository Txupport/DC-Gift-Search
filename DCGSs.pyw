...
# 1. FACE SETTINGS (Exact Coordinates)
FACE_BOX = (445, 133, 657, 323)

# 2. SCANNER SETTINGS
# Target Beige: fee2c0 -> (254, 226, 192)
# We use a wider tolerance but skip the top 330 pixels to avoid noise.
TARGET_COLOR = (254, 226, 192)
TOLERANCE = 50 
...
def perform_search(search_type):
    ...
    for index, m in enumerate(MONSTER_DB):
        if search_type == "name" and query in m["Name"].lower():
            matches.append((index, m))
        elif search_type == "item" and query in m["Gifts"].lower():
            matches.append((index, m))
        ...
        for i, (index, m) in enumerate(matches):
            if index < len(all_images):
                ...
                try:
                    ...
                    # Search from Y=330 to avoid face noise, with 50px margins
                    left, top, right, bottom = width, height, 0, 0
                    found = False
                    for y in range(330, height - 10, 4):
                    ...
                    gift_photo = None
                    if found and (right - left) > 100:
                        # Pad by 2px if possible
                        gift_crop = pil_img.crop((max(0, left-2), max(0, top-2), min(width, right+2), min(height, bottom+2)))
                    ...
                    if gift_photo:
                        ...
                        text_results.insert(tk.END, "\n[Info Box detection failed - Manual check image suggested]")
                    
                except Exception as e:
                    text_results.insert(tk.END, f"[Image Processing Error: {e}] ")
            
            text_results.insert(tk.END, "\n\nExtracted Gift List\n\n", "text_header")
            raw_gifts = m['Gifts']
            if raw_gifts:
                gift_list = sorted([g.strip() for g in raw_gifts.split(',')], key=lambda s: s.lower())
                for j, gift in enumerate(gift_list):
                    tag = "highlight" if (search_type == "item" and query in gift.lower()) else ""
                    text_results.insert(tk.END, gift, tag)
                    if j < len(gift_list) - 1: text_results.insert(tk.END, ", ")
...
tk.Label(frame_top, text="Enter Search Term:", bg=COLOR_BG_MAIN, fg=COLOR_TEXT_WHITE, font=("Helvetica", 11)).pack(side=tk.LEFT)
...
entry_search.bind("<Return>", lambda e: perform_search("name")) 

tk.Button(frame_top, text="Search Monster", command=lambda: perform_search("name"), bg=COLOR_BTN_BG, fg=COLOR_TEXT_WHITE, font=("Helvetica", 10, "bold"), padx=10).pack(side=tk.LEFT, padx=5)
tk.Button(frame_top, text="Search Item", command=lambda: perform_search("item"), bg=COLOR_BTN_BG, fg=COLOR_TEXT_WHITE, font=("Helvetica", 10, "bold"), padx=10).pack(side=tk.LEFT, padx=5)

text_results = scrolledtext.ScrolledText(root, wrap=tk.WORD, padx=20, pady=20, bg=COLOR_BG_MAIN, fg=COLOR_TEXT_WHITE, font=("Helvetica", 11))
